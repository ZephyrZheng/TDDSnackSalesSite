		...
		if (order_list==null) {
			...
			alert("暂无数据");
				...
				for (var i = 0; i < order_list.length; i++) {					
					...
					//todo..获取订单项传入插入
					GetOrderItem(order_list[i].ord_id,order_list[i].ord_address, Q);
				};
				//Order_init();
				window.setTimeout( 
					function(){
						...
						if (Q=="Unconfirmed") {
							...
							//隐藏地址改成已确认
							$('.address_change_button_star').hide();							
							$("div.ordstate").html("已确认");
						}else if(Q=="paied"){
							//隐藏地址改成已支付
							$('.address_change_button_star').hide();							
							$("div.ordstate").html("已支付");
						}
					}, 
					2000
				);			
		...
		<div class="container">
			...
			<div class="collapse navbar-collapse" id="bar_menu_list">
				<ul class="nav navbar-nav">
					...
					<li class="bg2 navbar_menu_item actions1"><a href="shopcar.jsp" target="_blank">我的购物车</a></li>
							...
							<td class="order_show_check_all">
								...
								<div class="order_show_check_all_right">
									<div class="order_type_menu bg3">
										<button type="button" class="btn btn-info btn-xs button2" id="order_unconfirmed" onclick="ShowUnConfirmed()">未确认订单</button>
										<button class="btn btn-info btn-xs button2 active" id="order_confirmed" onclick="ShowConfirmed()">已确认订单</button>
										<button class="btn btn-info btn-xs button2 active" id="order_finish" onclick="Showpaied()">已完成订单</button>
									</div>
								</div>
							</td>
						</tr>
